...
Menu miscellaneousMenu = {
    ...
    {
        { "Switcher le titre hb. sur l'app actuelle.", METHOD, .method = &MiscellaneousMenu_SwitchBoot3dsxTargetTitle },
        { "Changer le combo du menu", METHOD, .method = &MiscellaneousMenu_ChangeMenuCombo },
        { "Demarrer InputRedirection", METHOD, .method = &MiscellaneousMenu_InputRedirection },
        { "Mettre a jour la date et l'heure via NTP", METHOD, .method = &MiscellaneousMenu_UpdateTimeDateNtp },
        { "Annuler le decalage du temps de l'utilisateur", METHOD, .method = &MiscellaneousMenu_NullifyUserTimeOffset },
        { "Dumper le firmware DSP", METHOD, .method = &MiscellaneousMenu_DumpDspFirm },
        {},
    }
};
...
void MiscellaneousMenu_SwitchBoot3dsxTargetTitle(void)
{
    ...
    if(compareTids(currentTid, HBLDR_DEFAULT_3DSX_TID))
    {
        ...
        {
            res = -1;
            strcpy(failureReason, "aucun processus trouve");
        }
    }
    else
    {
    ...
    if (compareTids(newTid, HBLDR_DEFAULT_3DSX_TID))
        miscellaneousMenu.items[0].title = "Switcher le titre hb. sur l'app actuelle.";
    else
        miscellaneousMenu.items[0].title = "Switcher le titre hb. sur " HBLDR_DEFAULT_3DSX_TITLE_NAME;
    ...
    do
    {
        ...
        if(R_SUCCEEDED(res))
            ...
            Draw_DrawFormattedString(10, 30, COLOR_WHITE, "Echec de l'operation (%s).", failureReason);
...
void MiscellaneousMenu_ChangeMenuCombo(void)
{
    ...
    posY = Draw_DrawFormattedString(10, 30, COLOR_WHITE, "Le combo actuel du menu est :  %s", comboStrOrig);
    posY = Draw_DrawString(10, posY + SPACING_Y, COLOR_WHITE, "Veuillez entre le nouveau combo :");
    ...
    do
    {
        ...
        posY = Draw_DrawFormattedString(10, 30, COLOR_WHITE, "Le combo actuel du menu est :  %s", comboStrOrig);
        posY = Draw_DrawFormattedString(10, posY + SPACING_Y, COLOR_WHITE, "Veuillez entre le nouveau combo : %s", comboStr) + SPACING_Y;

        posY = Draw_DrawString(10, posY + SPACING_Y, COLOR_WHITE, "Nouveau combo defini !.");
...
void MiscellaneousMenu_InputRedirection(void)
{
    ...
    if(wasEnabled)
    {
        ...
        if(res != 0)
            sprintf(buf, "Echec de l'arret de InputRedirection (0x%08lx).", (u32)res);
        else
    ...
    do
    {
        ...
        if(!wasEnabled && cantStart)
            Draw_DrawString(10, 30, COLOR_WHITE, "Impossible de demarrer InputRedirection tant\nque le systeme n'a pas demarre.");
        else if(!wasEnabled)
        {
            ...
            if(!done)
            {
                ...
                if(res != 0)
                    ...
                    miscellaneousMenu.items[2].title = "Arreter InputRedirection";

                done = true;
            }

            if(res == 0)
                Draw_DrawString(10, 30, COLOR_WHITE, "Demarrage de InputRedirection... OK.");
            else
        ...
        else
        {
            ...
            {
                u32 posY = 30;
                posY = Draw_DrawString(10, posY, COLOR_WHITE, "InputRedirection arrete.\n\n");
                if (isN3DS)
                {
...
void MiscellaneousMenu_UpdateTimeDateNtp(void)
{
    ...
    do
    {
        ...
        posY = Draw_DrawFormattedString(10, posY + SPACING_Y, COLOR_WHITE, "Utilisez DPAD gauche/droite pour modifier\nle décalage horaire.\nUtilisez DPAD Haut/Bas pour changer les minutes.\nPressez A pour confirmer.") + SPACING_Y;
    ...
    do
    {
        ...
        if (cantStart)
            Draw_DrawFormattedString(10, posY + 2 * SPACING_Y, COLOR_WHITE, "Impossible de synchroniser la date et l'heure\ntant que le systeme n'a pas demarre.") + SPACING_Y;
        else if (R_FAILED(res))
            Draw_DrawFormattedString(10, posY + 2 * SPACING_Y, COLOR_WHITE, "Echec de l'operation (%08lx).", (u32)res) + SPACING_Y;
        else
            Draw_DrawFormattedString(10, posY + 2 * SPACING_Y, COLOR_WHITE, "Heure/date mises a jour.") + SPACING_Y;
...
void MiscellaneousMenu_NullifyUserTimeOffset(void)
{
    ...
    do
    {
        ...
        Draw_DrawString(10, 10, COLOR_TITLE, "Menu d'options diverses");
        if(R_SUCCEEDED(res))
            Draw_DrawString(10, 30, COLOR_WHITE, "Operation reussie.\n\nRedemarrez pour appliquer les changements.");
        else
...
void MiscellaneousMenu_DumpDspFirm(void)
{
    ...
    do
    {
        ...
        Draw_DrawString(10, 10, COLOR_TITLE, "Menu d'options diverses");
        if(R_SUCCEEDED(res))
            Draw_DrawString(10, 30, COLOR_WHITE, "Le firmware DSP. a ete copie dans /3ds/dspfirm.cdc\nsur la carte SD.");
        else
            Draw_DrawFormattedString(
                10, 30, COLOR_WHITE,
                "Echec de l'operation (0x%08lx).\n\nVerifiez que le menu Home est en marche\net que la carte SD est inseree.",
                res
            );
        Draw_FlushFramebuffer();
        Draw_Unlock();
